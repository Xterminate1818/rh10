...
class SnakeGameEnv(gym.Env):
	...
	def step(self, action):
		...
		#buffers for possible rewards, actual values set in env init
		...
		if self.apple_count >= SNAKE_LEN_GOAL:
		...
		if self.timesteps > self.maxStepsPerEpisode:
			print(f"Timed out")
			reward_for_timing_out = self.rewValue_for_timing_out
			self.terminated = True

		#produce a reward based on how close/far the agent is from the apple each time step...
		...
		#reward_for_delta_distance_to_apple = self.rewValue_for_delta_distance_to_apple * (1 / euclidean_dist_to_apple) #effect should be the closer the higher the reward, the farther the lower the reward.
		reward_for_delta_distance_to_apple = newValue #effect should be the closer the higher the reward, the farther the lower the reward.
	...
	def reset(self, seed=None, options=None):
		...
		self.timesteps = 0 #limit is 1000
		#endregion
