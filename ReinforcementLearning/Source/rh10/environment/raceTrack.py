...
class RaceTrack(gym.Env):
	...
	def __init__(self, dbg_report_dir: Optional[str] = None, render_run_str: Optional[str] = "Evaluation", render_mode: Optional[str] = "human"):
		...
		self.rewValue_for_grabbing_apple = 250
		self.timeCoefficient_for_hesitating_to_grab_apple = 0.2
		self.rewValue_for_NOT_grabbing_apple = 0.5
		self.rewValue_for_reaching_max_apples = 360
		self.rewValue_for_delta_distance_to_apple = 5
		self.scaleCoefficient_for_delta_distance_to_apple = 0.001
		self.rewValue_for_colliding_w_self = -2
		self.timeCoefficient_for_colliding_w_self = 0.25
		self.rewValue_for_colliding_w_bound = -2
		self.timeCoefficient_for_colliding_w_bound = 0.2
		...
		self.observation_space = spaces.Box(low=-500, high=500, shape=(5 + self.snake_len_pos_data,), dtype=np.int64)
	...
	def step(self, action):
		...
		self.ep_timesteps += 1
		self.apple_timesteps += 1 #keeps track of time it took to obtain an apple, resets on Reset() and on apple grab
		...
		reward_for_NOT_grabbing_apple = 0
		...
		if self.snake_head == self.apple_position:
			...
			_USE_DECAYING_APPLES = False
			if _USE_DECAYING_APPLES:
				reward_for_grabbing_apple = self.rewValue_for_grabbing_apple + (-1 * self.timeCoefficient_for_hesitating_to_grab_apple * self.ep_timesteps)		#fill the apple reward buffer

			_USE_APPLE_TIMESTEPS = True
			if _USE_APPLE_TIMESTEPS:
				if not self.apple_timesteps == 0:
					reward_for_grabbing_apple = self.rewValue_for_grabbing_apple * (3 / self.apple_timesteps)
				else:
					reward_for_grabbing_apple = self.rewValue_for_grabbing_apple
				self.apple_timesteps = 0
			...
			if self.dbg_enable:
				...
				if self.apple_count > self.DBG_checkpoint_high_score:
					self.DBG_checkpoint_high_score += 1
				...
				if self.DBG_checkpoint_high_score > self.DBG_best_HS:
					self.DBG_best_HS = self.DBG_checkpoint_high_score
			#endregion
		...
		_USE_DIST_BASED_REWARD = False
		#region <STEP - Distance-based Reward>
		if _USE_DIST_BASED_REWARD:
		...
		if self.apple_count >= self.snake_len_goal and self.terminated == False:
		...
		reward_for_this_step = reward_for_grabbing_apple + reward_for_NOT_grabbing_apple + reward_for_delta_distance_to_apple + reward_for_reaching_max_apples + reward_for_colliding_w_bound + reward_for_colliding_w_self + reward_for_timing_out
		...
		self.observation = [apple_pos_x, apple_pos_y] + list(snake_body_components_buffer) + [self.snake_body_actual_length, rel_delta_x_to_apple, rel_delta_y_to_apple]
		...
		self.info = {
			...
			"high_score" : self.DBG_checkpoint_high_score
		}
		#endregion
	...
	def reset(self, seed=None, options=None):
		...
		self.apple_timesteps = 0 #keeps track of time it took to obtain an apple, resets on Reset() and on apple grab
		#endregion
	...
	def DEBUG_PRINT_END_OF_TRAINING_DATA(self):
		...
		print(f"Best HS: {self.DBG_best_HS}")
	...
	def DEBUG_REPORT_END_OF_TRAINING_DATA(self):
		if not self.report_dir == None:
			try:
				...
				file.write(f"Best HS: {self.DBG_best_HS}")
